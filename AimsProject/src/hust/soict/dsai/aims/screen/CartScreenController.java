...
public class  CartScreenController {
	...
	private void initialize() {
		...
		if (cart.getItems().size() != 0){
			...
			tblMedia.getSelectionModel().select(cart.getItems().get(0));
			btnRemove.setVisible(true);
			if(cart.getItems().get(0) instanceof Playable) {
	...
	private void showFilteredMedia(String newValue) {
		FilteredList<Media> filteredData = new FilteredList<>(cart.getItems());
    ...
    void btnViewStorePressed(ActionEvent event) {
    	try {
    		final String STORE_FXML_FILE_PATH = "/hust/soict/soict/aims/screen.Store.fxml";
